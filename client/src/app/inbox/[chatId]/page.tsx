...
export default function ChatWindow() {
  ...
  const fetchRecipient = async () => {
    // Hämta chat info från Supabase (samma som innan)
    const { data: chat } = await supabase
      .from("chats")
      .select("user1_id, user2_id")
      .eq("id", safeChatId)
    ...
    // Hämta användare via backend istället för direkt från Supabase Auth
    const response = await fetch(`${API_URL}/api/users/${otherId}`);
