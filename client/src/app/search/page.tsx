...
export default function SearchPage() {
  ...
  // 🔥 Debounce (sök automatiskt efter 400ms)
  useEffect(() => {
    if (!query.trim()) {
    ...
    <div className="max-w-xl mx-auto p-4 pt-6">
      ...
      <div className="flex gap-2 mb-6 bg-white rounded-xl shadow px-3 py-2 border">
        ...
        <input
          ...
          placeholder="Sök användare eller inlägg..."
          value={query}
      ...
      {results.posts.length > 0 && !loading && (
        <div>
          <h2 className="text-xl font-semibold mb-3">Inlägg</h2>
          <div className="grid grid-cols-3 gap-2">
            {results.posts.map((p: any) => (
              <img
                key={p.id}
                src={p.image_url || "/default-post.png"}
